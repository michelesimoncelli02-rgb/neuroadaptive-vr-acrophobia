"""
Offline preprocessing, feature extraction, and classifier evaluation with a cross-subject approach.

For each participant, it:

1. Loads the recorded XDF file and converts it to an MNE Raw object.
2. Apply the same pipeline used in the offline_analysis.py.
3. Aggregates the extracted features across participants.
4. Evaluates multiple machine-learning classifiers using Leave-One-Subject-Out
   cross-validation to identify the best-performing offline fear classifier.

The script is intended for offline model development and to test whether a cross-subject
...
for participant in participants:
    ...
    for n_ep, epoch in enumerate(clean_epochs):
        ...
        # interpolate
        clean_epochs.interpolate_bads(reset_bads=True, verbose=False)
        '''
        print()
    ...
    # before starting to use an array structure (window)
    ...
    for n_win, window in enumerate(all_windows): # working on a window-scale
        ...
        # global FBA
        gFBA = np.mean([FBA_Fp1_2, FBA_F3_4, FBA_F7_8, FBA_FC5_6, FBA_FT9_10])
...
# %%
############# RESULTS, COMPARISONS AND SAVINGS #############
...
'''
results_file = Path(f"All_particicpants_classification_results.txt")
...
# Inspect performance per held-out subject
